...
from dataset.config import LEARNING_RATE, MODEL_DIR, PLOTS_DIR
...
# Só parâmetros treináveis no otimizador
optimizer = Adam(
...
if best_model:
    torch.save(best_model, os.path.join(MODEL_DIR, "best_resnet18.pth"))

elapsed = (time.time() - start) / 60
print(f"\n✅ ResNet18 finalizada — Melhor época: {best_epoch} | "
      f"Melhor val_acc: {best_acc:.4f} | Tempo: {elapsed:.1f} min")

# Gráficos
fig, axes = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle("Treinamento ResNet18 — BreaKHis")
...
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "resnet_training_curve.png"),
            dpi=300, bbox_inches="tight")
plt.show()

pd.DataFrame({"train_loss": train_losses, "val_loss": val_losses,
              "train_acc": train_accs,   "val_acc": val_accs}
             ).to_csv(os.path.join("results", "resnet_history.csv"), index=False)